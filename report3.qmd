...
**Estimands: **
We estimate the brier scores of the three settings, which represents a transportability-adjusted measure of the model's prediction accuracy in the simulated target population.

**Methods: **
The transportability Brier score of the simulated dataset is calculated in the same way we calculate the Brier score for the actual NHANES data.
...
To further investigate if we can assess transportibility without real data, using only summary statistics of the data, we should do the same analysis with different datasets besides NHANES, preferably with a true outcome variable.
